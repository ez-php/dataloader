...
```
src/
  DataLoader.php              — batches load() calls into one call to a batch load
                                 function; per-key memoization cache; prime/clear/clearAll
  Deferred.php                — placeholder for a value resolved by its owning
                                 DataLoader's dispatch(); triggers that dispatch lazily
                                 from get() if not yet resolved
  Exception/
    MissingKeyException.php   — thrown from Deferred::get() when the batch function's
                                 return value omitted the requested key
tests/
  TestCase.php                 — module's PHPUnit base class
  DataLoaderTest.php            — batching, dedup, memoization, prime/clear, missing-key
  DeferredTest.php               — resolved()/get() on an ownerless Deferred
```
...
- **`DataLoader`** — the entry point. `load(key)` queues a key and returns a `Deferred`
  (reusing the pending/cached one for a key already queued/resolved); `loadMany(keys)` is
  the same for a list. `dispatch()` runs the batch function once over every currently
  pending key and resolves each key's `Deferred`. `prime()`/`clear()`/`clearAll()` manage
  the per-key cache directly, without going through the batch function.
- **`Deferred`** — a lazy value placeholder. `get()` triggers its owning `DataLoader`'s
  `dispatch()` if the value isn't resolved yet, then returns it (or rethrows a rejection).
  `Deferred::resolved()` builds an already-resolved, ownerless instance for cases where a
  value is available up front (e.g. `DataLoader::load()` returning a cache hit).
- **`Exception\MissingKeyException`** — the batch function's contract is to return a value
  for every key it was given; this is thrown when it doesn't.

---

## Design Decisions and Constraints

- **Pull-based dispatch, not push/async.** `load()` never calls the batch function. The
  batch only runs the first time something calls `Deferred::get()` for a key that isn't
  resolved yet (or when `dispatch()` is called directly). This keeps the module fully
  synchronous — no event loop, no Fibers, no promise library — while still giving the
  "queue keys → resolve once → distribute" batching the idea behind this module asked for.
  Every value requested by the time `get()`/`dispatch()` fires is batched into one call;
  values requested afterwards start a new batch.
- **Batch function returns a map, not a parallel array.** `callable(list<int|string>
  $keys): array<int|string, mixed>` keyed by the same keys it was given, rather than the
  classic DataLoader convention of a same-length/same-order array. A keyed map is harder to
  get wrong (no silent off-by-one from a batch function that drops or reorders a key) and
  makes a truly missing key an explicit, checkable case (`MissingKeyException`) instead of
  undefined behavior.
- **No framework dependency.** This module depends on nothing but `php: ^8.5` — same as
  `ez-php/support`. It is deliberately usable outside `ez-php/framework` entirely (plain
  scripts, other frameworks), and inside `ez-php/graphql` resolvers is only one of its
  intended callers, not a dependency.
- **Attached as a git submodule**, not scaffolded from `make_module.php`'s generated-package
  path — see the module's own repository history for anything predating this monorepo's
  wiring.
...
- Pure unit tests, no infrastructure (no MySQL/Redis/Meilisearch) — every test constructs a
  `DataLoader` with an in-test closure as its batch function and asserts on call counts and
  resolved values.
- Test classes live in the shared `Tests\` namespace but must be uniquely named
  ...
  `DataLoader` when the obvious name is already taken.
...
|---|---|
| GraphQL-specific resolver wiring (e.g. a `DataLoaderRegistry` keyed by GraphQL field, request-scoped loader lifecycle tied to `GraphQLExecutor`) | `ez-php/graphql`, or the application's own resolver layer |
| Async/concurrent batch execution (parallel HTTP calls, Fiber-based scheduling) | Out of scope — this module is intentionally synchronous; a future async variant would be a separate class or package, not a change to `DataLoader`'s contract |
| ORM-specific eager-loading / relation batching | `ez-php/orm` (its own relation-loading mechanism); `DataLoader` is a generic primitive an ORM integration could be *built on*, not a replacement for one |